...
/*
Goals:
- Much faster perceived load (no long blocking preload loop)
- Progressive + prioritized frame loading (near current scroll)
- Smooth scroll → frame mapping (only draw on change)
- Fix invalid SVG attributes (fillRule / clipRule)
- Keep fade+scale hero + faster navbar fade

If you still need more speed: convert frames to .webp and reduce resolution.
*/
...
export default function ScrollFrames() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        const p = clamp01(total > 0 ? window.scrollY / total : 0);

        // Frame mapping
        const rawIndex = p * (TOTAL_FRAMES - 1);
        targetFrameRef.current = rawIndex;

        // Prioritize loading around the target frame (neighbors)
        ...
        if (p <= HERO_FADE_START) hOp = 1;
          ...
          hOp = 1 - (p - HERO_FADE_START) / (HERO_FADE_END - HERO_FADE_START);

        // Hero scale (smoothstep in fade window)
        let scale;
  ...
  useEffect(() => {
    ...
    const findBestLoaded = (preferred) => {
      // If target not loaded try walking backward then forward
      if (statusRef.current[preferred] === 2) return preferred;
      // backward search
      for (let b = preferred; b >= 0; b--) {
        if (statusRef.current[b] === 2) return b;
      }
      // forward fallback
      for (let f = preferred + 1; f < TOTAL_FRAMES; f++) {
